...
function Destruction({size, cards}) {
  // console.log('брюс', size.width)
 
  return (
 <div className="destruction">

{cards.map((card) => (
 <div key={card.name} className={`card ${card.is_potentially_hazardous_asteroid ? "card-dangerous" : "card-not-dangerous"}`}>
